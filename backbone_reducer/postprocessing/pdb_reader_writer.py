...
class PDB_Reader_Writer:
    ...
    def write_pdb(self, chains, file_name):
        ...
        for chain in chains:
            for i in range(len(chain.nodes)):
                if(len(chain.amino_acids)==0):
                    nodes_str.append(self.__format_node(chain.nodes[i], 'A', offset + i + 1))
                else:
                    nodes_str.append(self.__format_node_with_acid_amino(chain.nodes[i], chain.amino_acids[i],'A', offset + i + 1))
    ...
    @staticmethod 
    def __format_node_with_acid_amino(node,acid_amino,chain,n):
        """Encodes node to str in PDB format"""
        return \
            'ATOM      1  CA  ' + \
            acid_amino  + ' ' +\
