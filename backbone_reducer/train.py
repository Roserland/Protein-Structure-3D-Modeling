...
def eval_all(valid_dataloader, model, thres=0.5):
    ...
    with torch.no_grad():
        ...
        # tn = confusion_mat[0][0]; fn = confusion_mat[0][1]
        # fp = confusion_mat[1][0]; tp = confusion_mat[1][1]
        print("[Eval All]: \n", confusion_mat)
...
def valid(valid_dataloader,model ,criterion):
    with torch.no_grad():
        ...
        predictions_6 = torch.where(prob[:, 1:2, :, :, :] > 0.51, ones, zeros).squeeze(dim=1)
...
def train(args):
    ...
    train_dataset = CaBackboneSet(index_csv='../datas/backbone_reducer/train.csv', label_mode=args.backbone_mode,)
    valid_dataset = CaBackboneSet(index_csv='../datas/backbone_reducer/test.csv', label_mode=args.backbone_mode,)
