...
class PDB_Reader_Writer:
    ...
    def write_pdb(self, chains, file_name):
        ...
        for chain in chains:
            for i in range(len(chain.nodes)):
                # nodes_str.append(self.__format_node(chain.nodes[i], 'A', offset + i + 1))   # Ca
                nodes_str.append(self.__format_node_with_type(chain.nodes[i][1:4], 'A', "CA", 
                                                              AMINO_ACIDS[int(chain.nodes[i][0]) - 1], offset + i + 1))   # Ca
                nodes_str.append(self.__format_node_with_type(chain.nodes[i][4:7], 'A', "N", 
                                                              AMINO_ACIDS[int(chain.nodes[i][0]) - 1], offset + i + 1))   # N
                nodes_str.append(self.__format_node_with_type(chain.nodes[i][7:10], 'A', "C",
                                                              AMINO_ACIDS[int(chain.nodes[i][0]) - 1], offset + i + 1))   # C
                nodes_str.append(self.__format_node_with_type(chain.nodes[i][10:13], 'A', "O", 
                                                              AMINO_ACIDS[int(chain.nodes[i][0]) - 1], offset + i + 1))   # O
    ...
    @staticmethod
    def __format_node_with_type(node, chain, at_type, aa_type, n):
        """
        node: List[[List]]:   [[x, y, z], ...]
        chain: str, eg: "A"
        at_type: str, atoms type, in ["CA", "N", "C", "O"]
        aa_type: str, amino acid type, eg: GLY
        n: amino index, will be encoded then wrote to pdb_files
        """
        """Encodes node to str in PDB format"""
        if at_type == "CA":
            return \
            'ATOM      1  CA  {} '.format(aa_type) + \
            chain + \
            Hybrid36.hy36encode(4, n).rjust(4) + '    ' + \
            '{0:.3f}'.format(node[0]).rjust(8) + \
            '{0:.3f}'.format(node[1]).rjust(8) + \
