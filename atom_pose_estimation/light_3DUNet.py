...
# -*- encoding: utf-8 -*-
'''
@File    :   light_3DUNet.py    
@Contact :   roserland@gmail.com

A light 3D UNet module, simplified from original 3D UNet, the UNet here will reduce some 3D-Convolution Block, for the
amino-acid-cube is not so much complicated.

TODO: Reduce model stack_nums gradually

@Modify Time      @Author    @Version    @Desciption
------------      -------    --------    -----------
2022/2/9 10:33 下午   fanzw      1.0         None
'''
...
# segmentation model
class shallow_Modified3DUNet(nn.Module):
    def __init__(self, in_channels, n_classes, base_n_filter=16):
        super(Modified3DUNet, self).__init__()
        self.in_channels = in_channels
        self.n_classes = n_classes
...
class My_3D_CNN(nn.Module):
    def __init__(self, in_channels, base_n_filter=4):
        super(My_3D_CNN, self).__init__()
        self.in_channels = in_channels
        self.base_n_filter = base_n_filter
        ...
        self.fcn_layers = nn.Sequential(nn.Linear(1024, 256),
                                        nn.Linear(256, 64),)
        self.fcn_Ca_pos = nn.Linear(64, 3)
        self.fcn_N_pos = nn.Linear(64, 3)
        self.fcn_C_pos = nn.Linear(64, 3)
        self.fcn_O_pos = nn.Linear(64, 3)


        self.encoder_stage1 = nn.Sequential(
            nn.Conv3d(1, self.base_n_filter, 3, 1, padding=1),      # in_channel, out_channel, kernel_size, stride, padding
            nn.PReLU(self.base_n_filter),

            nn.Conv3d(self.base_n_filter, self.base_n_filter * 2, 3, 1, padding=1),
            nn.PReLU(self.base_n_filter * 2),
            nn.AvgPool3d(2, 2,)
        )

        self.encoder_stage2 = nn.Sequential(
            nn.Conv3d(self.base_n_filter * 2, self.base_n_filter * 2, 3, 1, padding=1),
            nn.PReLU(self.base_n_filter * 2),
        ...
        self.encoder_stage3 = nn.Sequential(
            nn.Conv3d(self.base_n_filter * 2, self.base_n_filter * 4, 3, 1, padding=1),
            nn.PReLU(self.base_n_filter * 4),

            nn.Conv3d(self.base_n_filter * 4, self.base_n_filter * 4, 3, 1, padding=1),
            nn.PReLU(self.base_n_filter * 4),
            nn.AvgPool3d(2, 2, )
        )
    ...
    def forward(self, x):
        out_1 = self.encoder_stage1(x)              # [64, 1, 32, 32, 32]
        out_2 = self.encoder_stage2(out_1)          # [64, 8, 16, 16, 16]
        out_3 = self.encoder_stage3(out_2)          # [64, 8, 8, 8, 8]
        out_3 = out_3.reshape(out_3.shape[0], -1)   # [64, 1024]
        out_4 = self.fcn_layers(out_3)              # [64, 64]

        Ca_output = self.fcn_Ca_pos(out_4)
        N_output = self.fcn_N_pos(out_4)
        C_output = self.fcn_C_pos(out_4)
        O_output = self.fcn_O_pos(out_4)
