...
class MultiHeadAttention(nn.Module):
    def __init__(self, heads, dmodel, d_k, d_v, dropout=0.2) -> None:
        ...
        self.d_v = d_v
        self.h = heads
        self.q_linear = nn.Linear(dmodel, heads * d_k)
        self.k_linear = nn.Linear(dmodel, heads * d_k)
        self.v_linear = nn.Linear(dmodel, heads * d_v)
    ...
    def forward(self, Q, K, V, attn_mask=None):
        ...
        print("Q,K,V shape:", K_.shape, Q_.shape, V_.shape, )
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, enc_input, slf_attn_mask=None):
        print("In Encoder Layer, attn_mask:", slf_attn_mask)
...
class Encoder(nn.Module):
    ...
    def forward(self, src_seq, src_mask, return_attns = False):
        ...
        print("In encoder, amino_type_vec shape: ", amino_type_vec.shape)
        fea_vec = torch.cat([amino_type_vec, src_seq[:, :, 1:]], dim=2)
        print("In encoder, the shape: ", fea_vec.shape)                     # [Bs, 512, 20]
...
class Decoder(nn.Module):
    ...
    def forward(self, trg_seq, trg_mask, enc_output, src_mask, return_attns=False):
        ...
        print("In encoder, the shape: ", fea_vec.shape)
...
class Transformer(nn.Module):
    ...
    def forward(self, src_seq, trg_seq):
        ...
        print("In transformer: dec_output shape", dec_output.shape)
        seq_logit = self.trg_amino_type_prj(dec_output)
        print("In transformer: seq_logit shape", seq_logit.shape)

        seq_pos = self.trg_amino_pos_prj(dec_output)
        print("In transformer: seq_pos shape", seq_pos.shape)

        if self.scale_prj:
            seq_logit *= self.d_model ** 0.5
        
        return seq_logit.view(-1, seq_logit.size(1)), seq_pos.view(-1, seq_pos.size(1))
...
if __name__ == '__main__': 
    ...
    for idx, data in enumerate(the_loader):
        ...
        trans_output = model(seq_data_array, labels)
