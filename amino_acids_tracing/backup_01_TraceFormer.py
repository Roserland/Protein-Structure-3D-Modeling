...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, Q, K, V, attn_mask=None):
        ...
        print(self.k_linear)
        tmp = self.k_linear(K)
        print("test vector size:", tmp.shape)
        ...
        print("K shapp: {}\t K_ shape: {}".format(K.shape, K_.shape))
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, enc_input, slf_attn_mask=None):
        ...
        print("Enc ouput size: ", enc_output.shape)
...
class Encoder(nn.Module):
    ...
    def forward(self, src_seq, src_mask, return_attns = False):
        ...
        print("\nIn Encoder Block, enc_output shape: ", enc_output.shape)
        return enc_output
...
class Decoder(nn.Module):
    ...
    def forward(self, trg_seq, trg_mask, enc_output, src_mask, return_attns=False):
        ...
        print("\nIn Decoder-Block: enc_ouput shape: ", enc_output.shape)
...
class Transformer(nn.Module):
    ...
    def forward(self, src_seq, trg_seq):
        ...
        print("\nIn Transformer Block: enc_output shape: ", enc_output.shape)
        dec_output = self.decoder(trg_seq, trg_mask, enc_output, src_mask)
        print("\nIn Transformer Block: dec_output shape: ", dec_output.shape)
        seq_logit = self.trg_amino_type_prj(dec_output)
        seq_pos = self.trg_amino_pos_prj(dec_output)
        print("\nIn Transformer Block: seq_logit shape: ", seq_logit.shape)
        print("\nIn Transformer Block: seq_pos shape: ", seq_pos.shape)
        if self.scale_prj:
